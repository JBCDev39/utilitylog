...
function statusBadge(s,inc,failType){
  var m={Fail:'b-fail',Vegetation:'b-veg','No Access':'b-na',Other:'b-other',Clean:'b-other'};
  var isDoorFail=s==='Fail'&&failType==='Rust Holes (Door)';
  var label=isDoorFail?'Door Fail':(s==='Clean'?'Other':s);
...
function renderMaps(){
  ...
  c.innerHTML=banner+db.maps.map(function(m,i){
    ...
    return '<div class="card card-anim" style="animation-delay:'+(i*0.04)+'s'+(isComplete?';opacity:0.65':'')+'"><div class="card-row" onclick="showUnits(\''+m.id+'\')" style="cursor:pointer"><div style="flex:1;min-width:0"><div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;flex-wrap:wrap"><div class="card-title">'+esc(m.name)+'</div>'+mapTypeBadge(m.type)+'<div class="map-status-pill '+(isComplete?'completed':'active')+'" onclick="toggleMapStatus(event,\''+m.id+'\')" ><div class="pill-dot"></div>'+(isComplete?'Completed':'Active')+'</div></div>'+( m.notes?'<div style="font-size:12px;color:var(--text3);margin-top:3px;font-style:italic;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(m.notes)+'</div>':''  )+'<div class="card-meta"><span>'+us.length+' unit'+(us.length!==1?'s':'')+'</span>'+(fails?'<span class="red">'+fails+' fail'+(fails>1?'s':'')+'</span>':'')+''+(doorFails?'<span style="color:var(--door-fail)">'+doorFails+' door fail'+(doorFails>1?'s':'')+'</span>':'')+''+(vegs?'<span style="color:var(--veg)">'+vegs+' veg</span>':'')+''+(patches?'<span>'+patches+' patch'+(patches>1?'es':'')+'</span>':'')+''+(incomplete?'<span class="warn">'+incomplete+' incomplete</span>':'')+'</div></div>'+thumb+'</div><div class=\"dot-menu-wrap\" style=\"position:absolute;bottom:10px;right:10px\"><button class=\"dot-btn\" onclick=\"event.stopPropagation();toggleMapDotById(event,\''+m.id+'\')\" aria-label=\"Map options\"><span></span><span></span><span></span></button><div class=\"dot-dropdown\" id=\"mapDD_'+m.id+'\"><div class=\"dot-item\" onclick=\"closeAllMapDots();editMapNotes(\''+m.id+'\')\">Notes</div><div class=\"dot-item\" onclick=\"closeAllMapDots();addMapPhoto(\''+m.id+'\')\">Photo</div><div class=\"dot-item\" style=\"color:var(--fail)\" onclick=\"closeAllMapDots();softDeleteMap(\''+m.id+'\',\''+esc(m.name)+'\')\">Delete</div></div></div></div></div></div>';
  }).join('')+dataSection();
}
...
function showTrash(){
  setTitle('Trash');$('topActs').innerHTML='';$('controlsRow').classList.remove('visible');hideFab();
  var c=$('screenTrash');
...
function showGallery(){
  setTitle('Gallery');$('topActs').innerHTML='';$('controlsRow').classList.remove('visible');hideFab();
...
function showUnit(id){
  ...
  setTitle(u.epcor);$('controlsRow').classList.remove('visible');

...
function statusBadge(s,inc){var m={Fail:'b-fail',Vegetation:'b-veg','No Access':'b-na',Clean:'b-clean'};return '<span class="badge '+(m[s]||'b-clean')+'">'+esc(s)+'</span>'+(inc?' <span class="badge b-incomplete">Quick Add</span>':'');}
...
function openPicker(type){
  ...
  if(type==='filter'){opts=['All units','Fail','Vegetation','No Access','Clean'];title='Filter';current=state.filter==='All'?'All units':state.filter;}
  ...
  $('pickerOptions').innerHTML=opts.map(function(o){
    var sel=o===current;
    return '<div class="picker-option'+(sel?' selected':'')+'" onclick="selectPickerOption(\''+type+'\',\''+o.replace(/'/g,"\\'")+'\')">'
      +'<span>'+esc(o)+'</span>'+(sel?'<svg class="picker-check" width="18" height="14" viewBox="0 0 18 14" fill="none"><path d="M1 7l5 5L17 1" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/></svg>':'')+'</div>';
  }).join('');
...
function renderMaps(){
  ...
  var banner='<div class="stats-banner card-anim"><div class="stats-banner-title">Season overview</div><div class="stats-row">'
    ...
    +'</div>'+(g.totalVeg?'<div class="stats-divider"></div><div style="font-size:12px;color:var(--text2)">'+g.totalVeg+' vegetation · '+g.activeMaps+' active map'+(g.activeMaps!==1?'s':'')+'</div>':'')+'</div>';
  ...
  c.innerHTML=banner+db.maps.map(function(m,i){
    ...
    var cam='<svg width="22" height="22" viewBox="0 0 20 20" fill="none" style="color:var(--text3)"><path d="M2 7a2 2 0 012-2h.5l1-2h9l1 2H16a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V7z" stroke="currentColor" stroke-width="1.5"/><circle cx="10" cy="11" r="3" stroke="currentColor" stroke-width="1.5"/></svg>';
    var thumb=m.photo?'<img class="map-thumb" src="'+m.photo+'" onclick="viewMapPhoto(\''+m.id+'\')" alt="Map">':'<div class="map-thumb-ph" onclick="addMapPhoto(\''+m.id+'\')">'+cam+'</div>';
    return '<div class="card card-anim" style="animation-delay:'+(i*0.04)+'s'+(isComplete?';opacity:0.65':'')+'"><div class="card-row" onclick="showUnits(\''+m.id+'\')" style="cursor:pointer"><div style="flex:1;min-width:0"><div style="display:flex;align-items:center;gap:8px;margin-bottom:4px"><div class="card-title">'+esc(m.name)+'</div><div class="map-status-pill '+(isComplete?'completed':'active')+'" onclick="toggleMapStatus(event,\''+m.id+'\')"><div class="pill-dot"></div>'+(isComplete?'Completed':'Active')+'</div></div><div class="card-sub">'+esc(m.location)+(date?' · '+date:'')+'</div>'+(m.notes?'<div style="font-size:12px;color:var(--text3);margin-top:3px;font-style:italic;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">'+esc(m.notes)+'</div>':'')+'<div class="card-meta"><span>'+us.length+' unit'+(us.length!==1?'s':'')+'</span>'+(fails?'<span class="red">'+fails+' fail'+(fails>1?'s':'')+'</span>':'')+(vegs?'<span class="grn">'+vegs+' veg</span>':'')+(patches?'<span>'+patches+' patch'+(patches>1?'es':'')+'</span>':'')+'</div></div>'+thumb+'</div><div class="map-acts"><button class="btn btn-sm" onclick="showUnits(\''+m.id+'\')">Open</button><button class="btn btn-sm" onclick="addMapPhoto(\''+m.id+'\')">Photo</button><button class="btn btn-sm" onclick="editMapNotes(\''+m.id+'\')">Notes</button><button class="btn btn-sm" onclick="softDeleteMap(\''+m.id+'\',\''+esc(m.name)+'\')">Delete</button></div></div>';
  }).join('')+dataSection();
}
...
function toggleMapStatus(e,mapId){e.stopPropagation();var m=db.maps.find(function(x){return x.id===mapId;});m.status=m.status==='Completed'?'Active':'Completed';idbSave();renderMaps();toast(m.status==='Completed'?'Marked as completed':'Marked as active');}
...
function showUnits(mapId,dir){
  ...
  $('topActs').innerHTML='<button class="btn btn-sm" onclick="showGallery()">Gallery</button><button class="btn btn-sm" onclick="showSummary()">Summary</button><button class="btn btn-sm" onclick="exportPDF()">PDF</button>';
...
function renderUnits(){
  ...
  var c=$('screenUnits');
  if(!us.length){c.innerHTML='<div class="empty-state"><div class="empty-title">'+(q?'No results':'No units yet')+'</div><div class="empty-sub">'+(q||state.filter!=='All'?'Try a different search or filter':'Tap + to add a unit')+'</div></div>';return;}
  c.innerHTML=us.map(function(u,i){
    ...
    return '<div class="card card-tap card-anim" style="animation-delay:'+(i*0.03)+'s" onclick="showUnit(\''+u.id+'\')">'
      +'<div class="card-row"><span class="card-title" style="font-family:monospace;font-size:14px">'+esc(u.epcor)+'</span>'+statusBadge(u.status,u.incomplete)+'</div>'
...
function detailPhotoSlot(u,key,label){
  var p=u[key+'Photo'];
  var camSVG='<svg width="20" height="20" viewBox="0 0 20 20" fill="none" style="color:var(--grn-text)"><path d="M2 7a2 2 0 012-2h.5l1-2h9l1 2H16a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V7z" stroke="currentColor" stroke-width="1.5"/><circle cx="10" cy="11" r="3" stroke="currentColor" stroke-width="1.5"/></svg>';
  if(p)return '<div class="photo-slot" onclick="viewDetailPhoto(\''+u.id+'\',\''+key+'\')" ><img src="'+p+'" alt="'+label+'"><span style="position:absolute;bottom:6px;left:8px;background:rgba(0,0,0,0.6);color:#fff;padding:3px 8px;border-radius:6px;font-size:10px;font-weight:600;z-index:1;letter-spacing:0.04em">'+label.toUpperCase()+'</span></div>';
...
function detailLaunch(unitId,key,cam){
  ...
  inp.onchange=function(){var file=inp.files[0];if(!file)return;var r=new FileReader();r.onload=function(e){compressImage(e.target.result,1600,0.85,function(comp){var u=db.units.find(function(x){return x.id===unitId;});u[key+'Photo']=comp;idbSave();showAnnotationModal(comp,unitId,key);});};r.readAsDataURL(file);};inp.click();
}
...
function removeDetailPhoto(unitId,key){var u=db.units.find(function(x){return x.id===unitId;});delete u[key+'Photo'];idbSave();closeModal();renderUnitDetail(db.units.find(function(x){return x.id===state.unitId;}));toast('Photo removed');}
...
function applyAnnotation(unitId,key){
  ...
  var scaleX=canvas.width/(wrap?wrap.clientWidth:1);
  var ctx=canvas.getContext('2d');var tx=annState.textX*scaleX;var ty=annState.textY*scaleX;
  var fontSize=Math.round(annState.fontSize*scaleX);
  ctx.font='bold '+fontSize+'px -apple-system,sans-serif';var metrics=ctx.measureText(text||' ');
  var pad=fontSize*0.4;var bw=metrics.width+pad*2;var bh=fontSize+pad*1.5;
  ctx.fillStyle='rgba(0,0,0,0.72)';roundRect(ctx,tx,ty,bw,bh,fontSize*0.25);ctx.fill();
  ctx.fillStyle='#ffffff';ctx.fillText(text,tx+pad,ty+fontSize+pad*0.3);
...
function stampGPSForm(){
  if(!navigator.geolocation){toast('GPS not available');return;}toast('Getting location…');
  navigator.geolocation.getCurrentPosition(function(pos){formGPS={lat:pos.coords.latitude,lng:pos.coords.longitude};var lbl=$('gpsFormLabel');if(lbl)lbl.textContent='Lat '+formGPS.lat.toFixed(5)+' · Lon '+formGPS.lng.toFixed(5);toast('Location stamped');},function(){toast('Could not get location');},{enableHighAccuracy:true,timeout:10000});
}
...
function startEditUnit(id){
  ...
  patchCount=u.patches||0;formPhotos={before:u.beforePhoto||null,after:u.afterPhoto||null};
  formGPS=u.lat?{lat:u.lat,lng:u.lng}:null;
  formState={mode:'edit',unitId:id,pendingPhotoKey:null,failTypeVal:u.failType||FAIL_TYPES[0]};
  openUnitForm(u,null);
}
function openUnitForm(u,saved){
  ...
  var chevSVG='<svg class="form-picker-chevron" width="12" height="7" viewBox="0 0 12 7" fill="none"><path d="M1 1l5 5 5-5" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"/></svg>';
  var html='<p class="modal-title">'+(u&&!saved?'Edit '+esc(u.epcor):'Add unit')+'</p>'
    +'<div class="form-group"><label class="form-label">EPCOR #</label><input id="uEpcor" value="'+(saved?esc(saved.epcor):(u?esc(u.epcor):''))+'" placeholder="'+(unitType==='Pedestal'?'e.g. PED15828':'e.g. T1234')+'" autocomplete="off" autocorrect="off" spellcheck="false" oninput="onEpcorInput()"/></div>'
    +'<div class="form-group"><label class="form-label">ASAP # (internal)</label><input id="uAsap" type="number" value="'+(saved?saved.asap:(u&&u.asap?u.asap:''))+'" placeholder="e.g. 33"/></div>'
    ...
    +'<div class="form-group"><label class="form-label">Fail type</label><button class="form-picker" onclick="openPicker(\'failType\')"><span class="form-picker-val" id="failTypeDisplay">'+esc(failTypeVal)+'</span>'+chevSVG+'</button></div>'
    ...
    +'<div class="form-group"><label class="form-label">Before photo</label><div class="photo-form-wrap'+(val('uEpcor')||u?'':' blocked')+'" id="formPhoto_before">'+formPhotoSlotInner('before')+'</div><p class="form-hint" id="photoHint" style="display:'+(val('uEpcor')||u?'none':'block')+'">Enter EPCOR # first to enable photos</p></div>'
    +'<div class="form-group"><label class="form-label">After photo</label><div class="photo-form-wrap'+(val('uEpcor')||u?'':' blocked')+'" id="formPhoto_after">'+formPhotoSlotInner('after')+'</div></div>'
  ...
  openModal(html);if(saved)patchCount=saved.patches||0;
}
...
function submitUnitForm(){
  ...
  if(formState.mode==='edit'){
    ...
    u.epcor=epcor;u.asap=val('uAsap').trim();u.unitType=activeInSeg('uTypeSeg');u.status=status;
    u.failType=isFail?failType:'';u.patches=isFail?patchCount:0;u.notes=val('uNotes').trim();u.fins=fins;u.incomplete=false;
    ...
    idbSave();clearFormState();closeModal();$('topTitle').textContent=esc(u.epcor);renderUnitDetail(u);toast('Saved');
  } else {
    var nu={id:uid(),mapId:state.mapId,epcor:epcor,asap:val('uAsap').trim(),unitType:activeInSeg('uTypeSeg'),status:status,failType:isFail?failType:'',patches:isFail?patchCount:0,notes:val('uNotes').trim(),fins:fins,incomplete:false,createdAt:Date.now()};
    ...
    db.units.push(nu);idbSave();clearFormState();closeModal();patchCount=0;formPhotos={before:null,after:null};formGPS=null;renderUnits();toast('Unit added');
  }
}
...
function submitQuickAdd(){var epcor=val('qaEpcor').trim();if(!epcor||!qaStatus)return;db.units.push({id:uid(),mapId:state.mapId,epcor:epcor,asap:'',unitType:'Pedestal',status:qaStatus,failType:'',patches:0,notes:'',incomplete:true,createdAt:Date.now()});idbSave();closeModal();renderUnits();toast(epcor+' added');}

function showNewMapModal(){
  newMapPhoto=null;
  var camSVG='<svg width="22" height="22" viewBox="0 0 20 20" fill="none" style="color:var(--grn-text)"><path d="M2 7a2 2 0 012-2h.5l1-2h9l1 2H16a2 2 0 012 2v8a2 2 0 01-2 2H4a2 2 0 01-2-2V7z" stroke="currentColor" stroke-width="1.5"/><circle cx="10" cy="11" r="3" stroke="currentColor" stroke-width="1.5"/></svg>';
  openModal('<p class="modal-title">New map</p><div class="form-group"><label class="form-label">Map name</label><input id="mName" placeholder="e.g. Sakaw West" autocomplete="off"/></div><div class="form-group"><label class="form-label">Location / area</label><input id="mLoc" placeholder="e.g. Edmonton North" autocomplete="off"/></div><div class="form-group"><label class="form-label">Notes (optional)</label><textarea id="mNotes" rows="2" placeholder="Gate code, contact info…"></textarea></div><div class="form-group"><label class="form-label">Primary unit type</label><div class="seg" id="mTypeSeg"><button class="active" onclick="segSel(\'mTypeSeg\',this)">Pedestal</button><button onclick="segSel(\'mTypeSeg\',this)">Transformer</button><button onclick="segSel(\'mTypeSeg\',this)">Mixed</button></div></div><div class="form-group"><label class="form-label">Map photo (optional)</label><div class="photo-form-wrap" id="newMapPhotoSlot"><div class="photo-form-empty" onclick="pickNewMapPhoto(false)">'+camSVG+'<span class="pfe-lbl">Tap to add</span></div><div class="photo-form-actions"><button onclick="pickNewMapPhoto(true)">Camera</button><button onclick="pickNewMapPhoto(false)">Gallery</button></div></div></div><button class="btn btn-primary" style="width:100%;padding:14px;font-size:15px" onclick="createMap()">Create map</button>');
...
function createMap(){var name=val('mName').trim();if(!name)return;var m={id:uid(),name:name,location:val('mLoc').trim(),notes:val('mNotes').trim(),type:activeInSeg('mTypeSeg'),status:'Active',createdAt:Date.now()};if(newMapPhoto)m.photo=newMapPhoto;newMapPhoto=null;db.maps.push(m);idbSave();closeModal();renderMaps();toast('Map created');}
...
function exportPDF(){
  ...
  doc.autoTable({startY:y,head:[['EPCOR #','ASAP #','Type','Status','Fail type','Patches','Fins','Notes']],body:us.map(function(u){return [u.epcor||'',u.asap||'',u.unitType||'',u.status+(u.incomplete?' *':''),u.failType||'—',u.patches||0,u.fins?'Yes':'',u.notes||''];}),styles:{fontSize:8,cellPadding:2.5,overflow:'linebreak'},headStyles:{fillColor:[21,128,61],textColor:255,fontStyle:'bold',fontSize:8},alternateRowStyles:{fillColor:[240,253,244]},columnStyles:{0:{fontStyle:'bold',cellWidth:24},1:{cellWidth:14,halign:'center'},2:{cellWidth:18},3:{cellWidth:16},4:{cellWidth:30},5:{cellWidth:12,halign:'center'},6:{cellWidth:10,halign:'center'},7:{cellWidth:'auto'}},didParseCell:function(d){if(d.section==='body'&&d.column.index===3){var s=d.cell.raw;if(s.indexOf('Fail')>-1)d.cell.styles.textColor=[180,30,30];else if(s==='Vegetation')d.cell.styles.textColor=[21,128,61];else if(s==='No Access')d.cell.styles.textColor=[100,100,100];}},margin:{left:14,right:14}});
  var wp=us.filter(function(u){return u.beforePhoto||u.afterPhoto;});
  if(wp.length){
    doc.addPage();doc.setFontSize(14);doc.setFont(undefined,'bold');doc.setTextColor(0);doc.text('Photos',14,20);var py=28;
    var imgW=(pw-14-14-6)/2;var imgH=imgW*0.65;
    wp.forEach(function(u){
      if(py+imgH+14>doc.internal.pageSize.getHeight()-14){doc.addPage();py=20;}
      doc.setFontSize(10);doc.setFont(undefined,'bold');doc.text(u.epcor+(u.asap?' · ASAP #'+u.asap:''),14,py);doc.setFont(undefined,'normal');py+=5;
      if(u.beforePhoto){try{doc.addImage(u.beforePhoto,'JPEG',14,py,imgW,imgH);}catch(e){}doc.setFontSize(8);doc.setTextColor(100);doc.text('Before',14,py+imgH+3);doc.setTextColor(0);}
...
function exportUnitPDF(id){
  ...
  if(u.beforePhoto||u.afterPhoto){
    y+=4;doc.setFontSize(11);doc.setFont(undefined,'bold');doc.text('Photos',14,y);y+=6;doc.setFont(undefined,'normal');
    var imgW=(pw-14-14-6)/2;var imgH=imgW*0.72;
    if(u.beforePhoto){try{doc.addImage(u.beforePhoto,'JPEG',14,y,imgW,imgH);}catch(e){}doc.setFontSize(8);doc.setTextColor(100);doc.text('Before',14,y+imgH+3);}
    if(u.afterPhoto){var ax=14+(u.beforePhoto?imgW+6:0);try{doc.addImage(u.afterPhoto,'JPEG',ax,y,imgW,imgH);}catch(e){}doc.setFontSize(8);doc.setTextColor(100);doc.text('After',ax,y+imgH+3);}
  }
